...
impl State {
    ...
    /// 查询刷新、应用与文件搜索的结果落地/过期、Alt+数字）以及一次性的交互动作
    /// （Alt+数字、Enter 启动、Esc 隐藏、右键菜单、文件模式切换、设置页开合）。
    ///
    /// 开关关闭时直接返回：既不构造消息也不落盘，所以交互路径上不再有同步文件写入。
    /// 窗口唤起类事件（`show before window ready`、`show issued`）、托盘/设置页派发的
    /// 扫描与退出命令、以及后台索引/图标/更新事件仍走 `plog`：它们不随按键重复，
    /// 也不由窗口内输入触发。本方法不参与结果、顺序或缓存的任何判定。
