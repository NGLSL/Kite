...
fn body(state: &State) -> Element<'_, Message> {
    let content: Element<'_, Message> = match state.settings_section {
        ...
        Section::SearchEngine => search_engine_card(state),
...
/// 搜索引擎分区：预设 chips 纵向排布，自定义模板独占一行。
fn search_engine_card(state: &State) -> Element<'static, Message> {
    ...
    let mut rows: Vec<Element<'static, Message>> = vec![
        container(column![
            text("默认引擎").size(13.5).color(TEXT).font(name_font()),
    ...
    if engine == "custom" {
        ...
        rows.push(container(column![
            text("URL 模板").size(13.5).color(TEXT).font(name_font()),
    ...
    rows.push(container(column![
        text("网页搜索快捷键").size(13.5).color(TEXT).font(name_font()),
