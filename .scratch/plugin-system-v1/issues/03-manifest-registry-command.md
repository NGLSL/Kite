...
- [ ] 读取插件目录下的 Manifest，解析 id/name/version/compatibility/runtime/contributes
- [ ] 校验：schema_version、plugin_api 兼容、Plugin ID 唯一与字符集、command/icon/资源路径必须落在插件根内
- [ ] idle_timeout 请求值 clamp 到 15–300 秒量级；禁止「0 = 永不关闭」语义
- [ ] Command 注册进 Core Index/搜索可见结果：可按 title/keywords 命中，结果 source 体现插件来源
- [ ] Command 未被选择前不得 spawn 插件可执行文件
- [ ] 普通 Query 路径：即使已安装多个插件也保持与无插件基本一致（0 插件进程）
- [ ] 路由/注册层单测覆盖：合法 Manifest、路径逃逸、不兼容 API、重复 ID、idle clamp
- [ ] `cargo test` 全绿
