...
const cargoHome = env.CARGO_HOME || path.join(env.USERPROFILE || env.HOME || "", ".cargo");
...
if (existsSync(cargoBin)) {
  if (pathKey) {
    const has = env[pathKey]
      .split(";")
      .some((entry) => entry.trim().toLowerCase() === cargoBin.toLowerCase());
    if (!has) env[pathKey] = `${cargoBin};${env[pathKey]}`;
  } else {
    env.PATH = cargoBin;
  }
}
